...
tx2gene <- readRDS(here("phase2/data/tx2gene_annotated.RDS"))

# Subset the transcript-to-gene annotation data to keep only valid transcripts
tx2gene <- tx2gene[tx2gene$keep, ]
transcripts <- str_glue_data(tx2gene, "{transcript_id} {gene_id}")
...
saveRDS(cds_info, snakemake@output)
